...
const pageFromQuery = route.query.page && !Number.isNaN(Number(route.query.page)) ? Number(route.query.page) : 1
const page = ref(pageFromQuery)

function isPrintablePage(pageNumber: number) {
    return pageNumber === page.value || pageNumber + PAGES_AROUND_CURRENT === page.value || pageNumber - PAGES_AROUND_CURRENT === page.value
}
...
<template>
    <div class="w-full flex justify-center items-center p-2 text-lg z-20">
        <button v-if="page !== 1" class="mr-2 bg-complementary text-dominant rounded w-6 h-6" @click="toPage(page - 1)">
            <ChevronLeftIcon />
        </button>
